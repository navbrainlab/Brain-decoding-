...
def setup_model(args, config):
    ...
    model = MultiScale1DCNN_v2(128,out_channels=config.out_channels, num_class=4, scales=config.scales).to(device)
...
def parse_args():
    ...
    parser.add_argument('--tag', type=str, default='sweep_knums_multiscale1d_v2 new_xzd_data_m1s1')
...
if __name__ == '__main__':
    ...
    def load_yaml_config(file_path: str) -> dict:
        ...
        return config
    # 示例：加载sweep_config.yaml
    sweep_config = load_yaml_config('wandb_sweep_confg/kernel_nums_1d.yaml')
    sweep_id = wandb.sweep(
        sweep=sweep_config,       # 上一步加载的配置字典
        project="BCI",  # 替换为你的项目名
    )
