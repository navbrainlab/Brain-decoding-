...
def get_mapping(root):
    ...
    x_pos = np.arange(ele_mapping.shape[0])
    y_pos = np.arange(ele_mapping.shape[1])
    ...
    ele_mapping[-1, 0] = -1
    ele_mapping[-1, -1] = -1
    ele_mapping = ele_mapping.reshape(-1)
    ele_mapping = ele_mapping[ele_mapping != -1].astype(np.int32)

    xx = xx.reshape(-1)[ele_mapping]
    yy = yy.reshape(-1)[ele_mapping]

    xx = torch.from_numpy(xx).long()
    yy = torch.from_numpy(yy).long()
    ele_mapping = torch.from_numpy(ele_mapping).long()
    ele_mapping = ele_mapping - 1
