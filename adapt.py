...
def main(args):
    ...
    logger = setup_logging(args.checkpoint_root, experiment_timestamp, args)
    # Save args used for this experiment
    args_path = os.path.join(args.checkpoint_root, f'args_{experiment_timestamp}.json')
    ...
    for target_date in args.target_dates:
        ...
        if 'daily_bdy' in args.data_path:
            try:
                indices = np.load(os.path.join(args.indices_root, f'results_{target_date}.npz'), allow_pickle=True)['indices']
            ...
            target_data, target_labels, _, _, indices = load_centered_data_for_otta(target_data_path, windows_size=256, step=args.ecog_step, train_ratio=1.0, return_date=False, indices=indices)
            ...
            try:
                ...
                logger.warning(f"Failed to load indices from {args.indices_root}: {e}. Setting indices=None")
                indices = None            
            target_data, target_labels, _, _, indices  = load_centered_bcic(subject_id, session=target_date, return_date=False, indices=indices, train_ratio=1.0)
        ...
        model_name = source_args.get('model_name', 'BaseNet')
        if model_name == 'BaseNet':
            model = BaseNet(
                ...
                dropout_inp=args.adapt_dropout_inp,
            ...
            raise ValueError(f"Unrecognized model name in source args: {model_name}")
        ...
        for data, label in tqdm(zip(target_data, target_labels), desc=f"Adapting on target domain {target_date}"):
            ...
            if new_item_num == 0 and args.update:
                ...
                model.train()
                for _ in range(args.adapt_steps):  # Single optimization step
                    outputs = model(x)
                    # MultiScale1DCNN_v2 returns (outs_list, features); use final branch output
            ...
            with torch.no_grad():
                ...
                if model_name == 'MultiScale1DCNN_v2':
                    ...
                    pred = outs[-1]
            # Record prediction
            prediction_list.append(pred.flatten().cpu().numpy())
            label_list.append(label)
            if torch.argmax(pred, dim=1).cpu().numpy() == np.argmax(label):
                num_correct += 1
            total_samples += 1
        ...
        results_path = os.path.join(args.checkpoint_root, f'adaptation_results_{target_date}.npz')
        record_dict = {
        ...
        adapted_model_path = os.path.join(args.checkpoint_root, f'adapted_model_{target_date}.npz')
        torch.save(model.state_dict(), adapted_model_path)

        # Plot the accuracy curve
        plt.figure()
        plt.plot(range(1, len(accuracy)+1), accuracy, label='OTTA Basenet')
        plt.xlabel('Number of Samples')
        plt.ylabel('Accuracy')
        plt.title(f'Online Adaptation Accuracy on Target Domain {target_date}')
        plt.legend()
        plt_path = os.path.join(args.checkpoint_root, f'adaptation_accuracy_{target_date}.png')
...
if __name__ == "__main__":
    ...
    # parser.add_argument('--indices_root', type=str, default='/media/ubuntu/Storage1/ecog_data/OTTA_results/20251231_142652',
    parser.add_argument('--indices_root', type=str, default='',
    ...
    parser.add_argument('--adapt_lr', type=float, default=1e-5, help='Learning rate for source domain training')
    parser.add_argument('--adapt_weight_decay', type=float, default=1e-5, help='Weight decay for source domain training')
    parser.add_argument('--adapt_dropout_inp', type=float, default=0.1, help='Input block Dropout rate for adaptation model')
    parser.add_argument('--adapt_dropout', type=float, default=0.1, help='Dropout rate for adaptation model')
    parser.add_argument('--adapt_steps', type=int, default=1, help='Number of optimization steps during each adaptation')
